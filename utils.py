...
def spatial_majority_vote(idx, coords, radius=200, min_frac=0.5, max_iter=1):
    """
    idx: (N,) 聚类标签（模型预测）
    coords: (N,2) 空间坐标（adata.obsm['spatial']）
    radius: 半径邻域（与你构图一致即可）
    min_frac: 邻域内某类别占比≥该阈值就改为该类（0.5=多数投票）
    max_iter: 可迭代多轮，通常 1 就够
    """
    idx = np.asarray(idx).copy()
    nbrs = NearestNeighbors(radius=radius, metric="euclidean", n_jobs=1).fit(coords)
